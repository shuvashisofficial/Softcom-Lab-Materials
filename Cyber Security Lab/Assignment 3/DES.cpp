...
std::string dec2bin(int num) {
    std::string res = std::bitset<32>(num).to_string(); // Assuming 32-bit integers
...
std::vector<int> initial_perm = {58, 50, 42, 34, 26, 18, 10, 2,
...
std::vector<int> exp_d = {32, 1, 2, 3, 4, 5, 4, 5,
                         6, 7, 8, 9, 8, 9, 10, 11,
...
std::string xorOperation(const std::string& a, const std::string& b) {
    if (a.length() != b.length()) {
        // Handle error: strings must have the same length for XOR operation
        std::cerr << "Error: Strings must have the same length for XOR operation." << std::endl;
        return "";
    }
...
std::vector<int> final_perm = {40, 8, 48, 16, 56, 24, 64, 32,
...
string encrypt(string pt, const std::vector<std::string>& rkb, const std::vector<std::string>& rk)
{
    pt = hex2bin(pt);
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        std::string xor_result = xorOperation(right_expanded, rkb[i]);

        // S-boxes: substituting the value from s-box table by calculating row and column
        std::string sbox_str = "";
        for (int j = 0; j < 8; ++j) {
        int row = bin2dec(stoi(xor_result.substr(j * 6, 1)) + stoi(xor_result.substr(j * 6 + 5, 1)));
        int col = bin2dec(stoi(xor_result.substr(j * 6 + 1, 4)));
        ...
        std::string result = xorOperation(left, sbox_str);
        left = result;
...
int main() {
    ...
    // Splitting
    std::string left = key.substr(0, 28); // rkb for RoundKeys in binary
    std::string right = key.substr(28, 28); // rk for RoundKeys in hexadecimal
    ...
    std::vector<std::string> rkb_rev(rkb.rbegin(), rkb.rend());
    std::vector<std::string> rk_rev(rk.rbegin(), rk.rend());
    std::string text = bin2hex(encrypt(cipher_text, rkb_rev, rk_rev));
