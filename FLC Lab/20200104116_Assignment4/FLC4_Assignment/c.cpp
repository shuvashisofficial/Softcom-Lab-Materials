...
int numbering(char *ptr, char (*str)[500])
{
int i = 0;
int j;
int k = 0;
...
{
int i, s;i = 0;s = 0;
if((ptr[i] == 'i' && ptr[i+1] == 'f') || (ptr[i]
== 'e' && ptr[i+1] == 'l' && ptr[i+2] == 's' &&
ptr[i+3] == 'e'))
{
return 0;
}
else if((ptr[i] <= 'z' && ptr[i] >= 'a') || (ptr[i]
<= 'Z' && ptr[i] >= 'A') || ptr[i] == '_')
{
s = 1;i++;
}
...
int token_check(char *ptr, int c, char
*token_char, int t_len)
{
int tc1 = c - 1;
int ic = t_len - 1;
int flag = 1;
...
for(int j = 0; j < 7; j++)
{
a = token_check(ptr, i, ichar[j],
strlen(ichar[j]));
if(a == 1)
{
ptr2[lex_t] = ptr[i];lex_t++;
}
}
}
else
...
ef_cout += a;
if(a < 0)
cout << "Unmatched else at line " << i+1
<< endl;
}
for(int i = 0; i < c; i++)
{
...
a = kw_error(num[i]);
if(a == 1)
cout << "Duplicate Keyword at line " <<
i+1 << endl;
}
}
